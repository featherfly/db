...
public final class JdbcUtils {
	 ...
	 * @param value 参数
	 */
	public static void setParameter(PreparedStatementWrapper prep,
		...
		if (value == null) {
			...
			prep.setObject(position, (java.util.Date) value);
		} else if (value.getClass().isEnum()) {
			prep.setInt(position, ((Enum<?>) value).ordinal());
		} else {
			LOGGER.warn("setParameter不支持的数据类型{}，忽略", value
					.getClass().getName());
		}
	}
	 ...
	 * @param value 参数
	 */
	public static void setParameter(PreparedStatement prep, int position,
			Object value) {
		try {
			if (value == null) {
				prep.setObject(position, value);
				...
				LOGGER.warn("setParameter不支持的数据类型{}，忽略",
